...
export const AdminDoctorsManage = () => {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      const doctorsList = response.filter((user) => user.role === "doctor");
      ...
      onOk: async () => {
        try {
          ...
          const updatedDoctors = doctors.filter(
            (doctor) => doctor.id !== record.id
          );
          setDoctors(updatedDoctors);
        } catch (error) {
          console.error("Error deleting doctor:", error);
          message.error("Xóa bác sĩ thất bại. Vui lòng thử lại!");
        } finally {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "address",
      render: (address) => (
        <span className="text-gray-600 text-sm">{address || "N/A"}</span>
      ),
      ellipsis: true,
    },
    {
  ...
  if (loading) {
    return (
      <AdminLayout activeMenu="admin/doctors" displayName="Doctor Management">
